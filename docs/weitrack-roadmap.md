...
> 版本：v1.0 ｜ 日期：2026-08-17 ｜ 数据源：weitrack.db events 表（1422 条，含 2026-08-17 上午实测数据）

---

## 1. 项目背景与目标

weiCheckApp（用机时长）采集 9 类手机事件，经 POST /ingest 上报到自建 FastAPI 后端（gacore.weitrack），落库 events 表。当前状态：**仅存不读**——数据是原始事件流，尚未加工成可分析的结构化事实。
...
1. **P1 清洗 + ETL**（✅ 已完成，`gacore/weitrack/etl.py`）
...
> 本文件为 weiTrack 数据链路的**路书**：既是指南也是工作记录。
