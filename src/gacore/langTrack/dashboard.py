...
def _render_persona(conn: sqlite3.Connection, days: int = 7) -> str:
    """C1 人物画像卡片：Traits / 屏幕健康 / 节奏标签 / 画像文本。"""
    try:
        p = build_persona(conn=conn, days=days)
    except Exception:
        return ""
    ...
    rh = p.get("rhythm", {})
    return (
        f'<div class="card"><h2>人物画像（近 {days} 天）</h2>'
...
def render_dashboard_html(conn: sqlite3.Connection, day: str | None = None) -> str:
    ...
    day = day or datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=8))).strftime("%Y-%m-%d")
